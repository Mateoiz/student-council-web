...
function LoginForm() {
  ...
  // Grab the redirect parameter, fallback to /admin if none exists
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Redirect dynamically based on the URL parameter
...
export default function AdminLoginPage() {
  return (
    <main className="min-h-screen bg-zinc-50 text-zinc-900">
      ...
      {/* 
        Suspense boundary is required when using useSearchParams() 
        to prevent layout shifting and build errors in Next.js 
      */}
      <Suspense fallback={
